...
// only admins/owners may access
if (!isset($_SESSION["user"]) ||
    (!in_array($_SESSION["user"]["role"], ['admin','owner']))) {
...
// back‐link config (if used elsewhere in your template)
$showBack = true;
$backLink = "admin_dashboard.php";

// ─── role maps for promote/demote buttons ──────────
$roleLabels = [
  'user'      => 'Candidate (User)',
  'scientist' => 'Scientist',
  'evaluator' => 'Evaluator',
      ...
      >
        <div class="d-flex flex-wrap gap-3">
          <div class="d-flex flex-column" style="min-width: 180px;">
            <label for="nameFilter" class="form-label mb-1">
              Filter by Name:
            </label>
            <input
              type="text"
          ...
          <div class="d-flex flex-column" style="min-width: 160px;">
            <label for="roleFilter" class="form-label mb-1">
              Filter by Role:
            </label>
          ...
          <div class="d-flex flex-column" style="min-width: 160px;">
            <label for="verifyFilter" class="form-label mb-1">
          ...
          >
            ...
            <tbody>
              <?php foreach ($users as $user): ?>
                <tr
                  class="role-<?= htmlspecialchars($user['role']) ?>
                         verify-<?= $user['is_verified'] ?>"
                  data-name="<?= strtolower(htmlspecialchars($user['full_name'])) ?>"
                >
                  <td><?= $user['id'] ?></td>
                  ...
                  <td>
                    ...
                    <?php else: ?>
                      ...
                      <!-- Promote button -->
                      <?php if (isset($promoteTo[$user['role']])):
                        $to = $promoteTo[$user['role']];
                      ?>
                        <a
                          href="update_role.php?id=<?= $user['id'] ?>&role=<?= $to ?>"
                          class="btn btn-sm btn-success"
                        >
                          Promote to <?= $roleLabels[$to] ?>
                        </a>
                      <?php endif; ?>

                      <!-- Demote button -->
                      <?php if (isset($demoteTo[$user['role']])):
                        $to = $demoteTo[$user['role']];
                      ?>
                        <a
                          href="update_role.php?id=<?= $user['id'] ?>&role=<?= $to ?>"
                          class="btn btn-sm btn-warning"
                        >
                          Demote to <?= $roleLabels[$to] ?>
                        </a>
                      <?php endif; ?>
    ...
    function filterUsers() {
      ...
      document.querySelectorAll('tbody tr').forEach(row => {
        ...
        const roleMatch   = roleValue === 'all' ||
                            row.classList.contains(`role-${roleValue}`);
        const verifyMatch = verifyValue === 'all' ||
                            row.classList.contains(`verify-${verifyValue}`);

        row.style.display = nameMatch && roleMatch && verifyMatch
          ? ''
          : 'none';
      });
    }
